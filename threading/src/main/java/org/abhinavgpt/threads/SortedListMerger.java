...
public class SortedListMerger implements Callable<List<Integer>> {
    private final List<Integer> listToBeSorted;

    public SortedListMerger(List<Integer> listToBeSorted) {
    ...
    private List<Integer> mergeSort(List<Integer> list) throws Exception {
        ...
        Future<List<Integer>> leftFuture = executorService.submit(new SortedListMerger(list.subList(0, mid)));
        Future<List<Integer>> rightFuture = executorService.submit(new SortedListMerger(list.subList(mid, list.size())));
